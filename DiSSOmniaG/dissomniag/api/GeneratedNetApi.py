...
log = logging.getLogger("api.GeneratedNetApi.py")

def isGeneratedNetNameValid(user, generatedNetName):
    # HACK until Name change supported always return false
    
    root = etree.Element("result")
    errorMsg = etree.SubElement(root, "error")
    
    generatedNetName = str(generatedNetName)
    ...
    log.info("Senf isGeneratedNetNameValid: " + retString)
    return retString